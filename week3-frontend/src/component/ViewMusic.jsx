...
const ViewMusic = () => {
  ...
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        const response = await axios.get('http://localhost:9090/api/songs/view');
  ...
  const handleDelete = async (songId) => {
    try {
      await axios.delete(`http://localhost:9090/api/songs/delete/${songId}`);
        ...
        {songs.map((song) => (
          <div key={song._id} className="song-card">
            ...
            <div className="song-details">
              <h3 style={{ color:"darkcyan" }}>{song.title}</h3>
